...
{
	/* we declare this variable to be volatile so that the compiler will not
	 * store it in a register. Otherwise, this code may or may not work as
	 * expected. */
	...
	printf("ucontext_t size = %ld bytes\n", (long int)sizeof(ucontext_t));      //changed
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (unsigned long)((unsigned long long)&err - mycontext.uc_mcontext.gregs[REG_RSP]));
